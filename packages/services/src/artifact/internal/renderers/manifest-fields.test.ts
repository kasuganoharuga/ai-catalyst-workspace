...
describe("resolveSectionCount", () => {
  it("returns a fixed count unchanged", () => {
    const manifest: FieldManifest = {
      sectionPrefix: "x",
      sectionCount: { kind: "fixed", value: 3 },
      fields: [],
    };
  ...
  it("accepts a requested count within an option manifest's range", () => {
    ...
    expect(resolveSectionCount(PROBLEM_INTERVIEW_FIELD_MANIFEST_V1, 10)).toBe(
      10,
    );
  });

  it.each([4, 11, 7.5])(
    "rejects an out-of-range or non-integer request (%s)",
    (requested) => {
      expect(() =>
        resolveSectionCount(PROBLEM_INTERVIEW_FIELD_MANIFEST_V1, requested),
      ).toThrow(/WORKBOOK_RENDER_FAILED/);
    },
  );

  it("accepts a requested count within a fromModel manifest's range", () => {
    expect(resolveSectionCount(VALIDATION_ROADMAP_FIELD_MANIFEST_V1, 2)).toBe(
      2,
    );
    expect(resolveSectionCount(VALIDATION_ROADMAP_FIELD_MANIFEST_V1, 3)).toBe(
      3,
    );
  });
  ...
  it("rejects a fromModel count outside its declared range", () => {
    expect(() =>
      resolveSectionCount(VALIDATION_ROADMAP_FIELD_MANIFEST_V1, 4),
    ).toThrow(/WORKBOOK_RENDER_FAILED/);
  });
});
...
describe("expectedFieldNames — problem_interview_workbook_v1", () => {
  ...
  it("names every section interview_1..interview_N regardless of round/additional status", () => {
    const names = expectedFieldNames(PROBLEM_INTERVIEW_FIELD_MANIFEST_V1, {
      ...context,
      sectionCount: 6,
    });
...
describe("manifest capacity discipline (plan §6, §12)", () => {
  it.each([
    ...
    "%s has no undefined or placeholder capacity on any text field",
    (_name, manifest) => {
      for (const field of allFieldSpecs(manifest)) {
        if (field.type !== "text") continue;
        expect(
          field.capacity,
          `field with suffix "${"suffix" in field ? field.suffix : field.suffixTemplate}"`,
        ).not.toBe(undefined);
