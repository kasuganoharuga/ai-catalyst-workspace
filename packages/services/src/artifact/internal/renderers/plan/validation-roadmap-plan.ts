...
import type {
  Experiment,
  ValidationRoadmapModel,
} from "../parse/validation-roadmap.js";
...
function coverPage(layout: LayoutBuilder, model: ValidationRoadmapModel): void {
  ...
  criterionLine(
    layout,
    "cover.time_available",
    "Time available",
    model.constraints.timeAvailable,
  );
  criterionLine(layout, "cover.budget", "Budget", model.constraints.budget);
  criterionLine(
    layout,
    "cover.customer_access",
    "Customer access",
    model.constraints.customerAccess,
  );

  sectionHeading(
    layout,
    "cover.what_these_experiments_test_heading",
    "What these experiments test",
  );
  layout.lockedText(
    "cover.what_these_experiments_test",
    model.whatTheseExperimentsTest,
    { size: 9 },
    { gap: 8 },
  );
  ...
  sectionHeading(
    layout,
    "cover.experiments_overview_heading",
    "Experiments overview",
  );
  model.experiments.forEach((experiment, i) => {
    const idx = i + 1;
    layout.lockedText(
      `cover.experiment_${idx}_name`,
      `${idx}. ${experiment.name}   (${experiment.window})`,
      { size: 9, bold: true },
      { gap: 2 },
    );
    criterionLine(
      layout,
      `cover.experiment_${idx}_claim`,
      "Claim tested",
      experiment.claimTested,
    );
    criterionLine(
      layout,
      `cover.experiment_${idx}_pass`,
      "Pass condition",
      experiment.passCondition,
    );
    criterionLine(
      layout,
      `cover.experiment_${idx}_fail`,
      "Fail condition",
  ...
  model.signalStrengthAnchors.forEach((anchor, i) => {
    layout.lockedText(
      `cover.signal_strength_anchor_${i + 1}`,
      anchor,
  ...
  criterionLine(
    layout,
    "cover.start_here_what_to_do",
    "What to do",
    model.startHere.whatToDo,
  );
  criterionLine(
    layout,
    "cover.start_here_who_to_contact",
    "Who to contact, and how",
    model.startHere.whoToContact,
  );
  criterionLine(
    layout,
    "cover.start_here_pass",
    "What counts as a pass",
    model.startHere.pass,
  );
  criterionLine(
    layout,
  ...
  sectionHeading(
    layout,
    "cover.how_to_record_results_heading",
    "How to record results",
  );
  layout.lockedText(
    "cover.how_to_record_results",
    model.howToRecordResults,
    { size: 9 },
    { gap: 0 },
  );
}
...
function experimentSection(
  ...
  criterionLine(
    layout,
    `${prefix}.pass_condition`,
    "Pass condition",
    experiment.passCondition,
  );
  criterionLine(
    layout,
    `${prefix}.fail_condition`,
    "Fail condition",
    experiment.failCondition,
  );
  ...
  if (index === 1) {
    ...
    criterionLine(
      layout,
      `${prefix}.start_here_what_to_do`,
      "What to do",
      model.startHere.whatToDo,
    );
    criterionLine(
      layout,
      `${prefix}.start_here_who_to_contact`,
      "Who to contact, and how",
      model.startHere.whoToContact,
    );
  }
  ...
  layout.textField(`${prefix}.participants`, {
  ...
  layout.textField(`${prefix}.contact_route`, {
    width: layout.usableWidth,
    height: 70,
    multiline: true,
    capacity: 360,
  ...
  layout.textField(`${prefix}.actions_completed`, {
    width: layout.usableWidth,
    height: 110,
    multiline: true,
    capacity: 640,
  ...
  layout.textField(`${prefix}.dates_completed`, {
    width: 200,
    height: 15,
    capacity: 60,
    gap: 0,
  ...
  layout.textField(`${prefix}.observable_result`, {
    width: layout.usableWidth,
    height: 110,
    multiline: true,
    capacity: 640,
  ...
  layout.textField(`${prefix}.verbatim_evidence`, {
    width: layout.usableWidth,
    height: 110,
    multiline: true,
    capacity: 640,
  ...
  layout.textFieldAt(
    `${prefix}.actual_time`,
    { x: layout.margin, y: timeY - 15, width: halfWidth, height: 15 },
    40,
  );
  layout.y = timeY;
  ...
  layout.dropdownAt(
    `${prefix}.outcome`,
    { x: layout.margin, y: layout.y - 18, width: 200, height: 18 },
    ["Pass", "Fail", "Inconclusive"],
  );
  layout.advance(18 + 8);

  labelLine(layout, "Effect on evidence maturity");
  layout.textField(`${prefix}.maturity_impact`, {
    width: layout.usableWidth,
    height: 70,
    multiline: true,
    capacity: 360,
  ...
  layout.textField(`${prefix}.next_action`, {
    width: layout.usableWidth,
    height: 90,
    multiline: true,
    capacity: 550,
...
export function buildValidationRoadmapPlan(
  ...
  model.experiments.forEach((experiment, i) =>
    experimentSection(layout, model, experiment, i + 1),
  );
 ...
 * the plan verbatim.
 */
export function assertValidationRoadmapPlan(
