...
function twoColumnCheckboxes(
  ...
  sectionHeading(
    layout,
    `interview_${sectionIndex}.pass_bar_heading`,
    "Pass Bar observed",
  );
...
function referencePage(
  ...
  sectionHeading(
    layout,
    "reference.interview_target_heading",
    "Interview target",
  );
  layout.lockedText(
    "reference.interview_target",
  ...
  sectionHeading(
    layout,
    "reference.what_this_tests_heading",
    "What this interview tests",
  );
  layout.lockedText(
    "reference.what_this_tests",
    model.whatThisInterviewTests,
    { size: 9 },
    { gap: 8 },
  );
  ...
  layout.lockedText(
    "reference.pass_bar_preamble",
    model.passBar.preamble,
    { size: 8.5, color: MUTED },
    { gap: 6 },
  );
  model.passBar.conditions.forEach((condition, i) => {
    layout.lockedText(
      `reference.pass_bar_condition_${i + 1}`,
      `P${i + 1}   ${condition}`,
      { size: 9 },
      { gap: 4 },
    );
  ...
  sectionHeading(
    layout,
    "reference.after_each_call_heading",
    "After each call",
  );
  model.afterEachCall.forEach((item, i) => {
    layout.lockedText(
      `reference.after_each_call_${i + 1}`,
      `•  ${item}`,
      { size: 9 },
      { gap: 3 },
    );
  ...
  sectionHeading(
    layout,
    "reference.where_results_go_heading",
    "Where results go",
  );
  layout.lockedText(
    "reference.where_results_go",
    model.whereResultsGo,
    { size: 9 },
    { gap: 0 },
  );
...
function interviewSection(
  ...
  const title = isAdditional
    ? `Additional interview ${index - ROUND_SIZE}`
    : `Interview ${index}`;
  ...
  sectionHeading(
    layout,
    `${prefix}.page_a_title`,
    isAdditional ? title : `${title} of ${ROUND_SIZE}`,
  );
  ...
  layout.textFieldAt(
    `${prefix}.date_day`,
    { x: layout.margin, y: dateY - 15, width: 40, height: 15 },
    2,
  );
  layout.textFieldAt(
    `${prefix}.date_month`,
    { x: layout.margin + 44, y: dateY - 15, width: 40, height: 15 },
    2,
  );
  layout.textFieldAt(
    `${prefix}.date_year`,
    { x: layout.margin + 88, y: dateY - 15, width: 56, height: 15 },
    4,
  );
  layout.advance(15 + 8);

  labelLine(layout, "Participant identifier");
  layout.textField(`${prefix}.participant`, {
  ...
  layout.textFieldAt(
    `${prefix}.role_organisation`,
    { x: layout.margin, y: roleY - 15, width: halfWidth, height: 15 },
    40,
  );
  layout.y = roleY;
  labelLine(
  ...
  layout.textField(`${prefix}.beachhead_match`, {
    width: layout.usableWidth,
    height: 48,
    multiline: true,
    capacity: 250,
  ...
  labelLine(
    layout,
    "Contradicting evidence — especially the inconvenient kind",
  );
 ...
 */
export function assertInterviewWorkbookPlan(
