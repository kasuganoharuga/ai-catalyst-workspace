...
function parseExperiments(markdown: string): Experiment[] {
  ...
  if (table === null) {
    fail(
      `'s "${EXPERIMENTS_HEADING}" section does not contain a Markdown table.`,
    );
  }
  if (table.rows.length < 2 || table.rows.length > 3) {
    fail(
      `'s "${EXPERIMENTS_HEADING}" table must have 2-3 rows, found ${table.rows.length}.`,
    );
  }

  const columnIndex: Record<(typeof REQUIRED_COLUMNS)[number], number> =
    {} as Record<(typeof REQUIRED_COLUMNS)[number], number>;
  for (const column of REQUIRED_COLUMNS) {
    const index = tableColumnIndex(table.headers, column);
    if (index === -1) {
      fail(
        `'s "${EXPERIMENTS_HEADING}" table is missing the "${column}" column.`,
      );
    }
  ...
  return table.rows.map((row, rowIndex) => {
    const cell = (column: (typeof REQUIRED_COLUMNS)[number]): string => {
      ...
      if (!isSubstantiveText(raw)) {
        fail(
          `'s "${EXPERIMENTS_HEADING}" table row ${rowIndex + 1} is missing "${column}".`,
        );
      }
...
export function parseValidationRoadmap(
  markdown: string,
  ...
  const constraints = {
    timeAvailable: requiredLabel(markdown, "Time available", {
      level: 2,
      heading: "Constraints",
    }),
    budget: requiredLabel(markdown, "Budget", {
      level: 2,
      heading: "Constraints",
    }),
    customerAccess: requiredLabel(markdown, "Customer access", {
      level: 2,
      heading: "Constraints",
    }),
  };

  const whatTheseExperimentsTest = requiredSection(
    markdown,
    2,
  ...
  const startHere = {
    whatToDo: requiredLabel(markdown, "What to do", {
      level: 2,
      heading: "Start Here",
    }),
    whoToContact: requiredLabel(markdown, "Who to contact, and how", {
      level: 2,
      heading: "Start Here",
    }),
    pass: requiredLabel(markdown, "What counts as a pass", {
      level: 2,
      heading: "Start Here",
    }),
    fail: requiredLabel(markdown, "What counts as a fail", {
      level: 2,
      heading: "Start Here",
    }),
  };
  ...
  if (
    normalizeComparisonValue(startHere.pass) !==
    normalizeComparisonValue(experiments[0].passCondition)
  ) {
    fail(
  ...
  if (
    normalizeComparisonValue(startHere.fail) !==
    normalizeComparisonValue(experiments[0].failCondition)
  ) {
    fail(
  ...
  return {
    ...
    experiments,
    signalStrengthAnchors: signalStrengthAnchors as [
      string,
      string,
      string,
      string,
