...
// Interview record CRUD and shared row mappers for evidence/module4.
...
export function asQuestions(raw: unknown): InterviewQuestionSnapshot[] {
...
export function asAnswers(raw: unknown): Record<string, string> {
...
export function mapActivity(row: ActivityRow): InterviewActivity {
  return {
    id: row.id,
...
export function mapRecord(row: RecordRow): InterviewRecord {
  const interviewedAt =
...
// --- Create activity on Module 3 confirm ---
...
/** Also used by evidence.ts's submit/confirm/reopen evidence mutations. */
export async function assertNoPinnedClaudeAttempt(
  workspaceId: string,
  programRunId: string,
  client: PoolClient,
): Promise<void> {
...
/**
 * Revert evidence to draft when records change after submit/confirm (if not pinned).
 */
...
/** Reopen completed record for editing; confirmed evidence returns to draft if unpinned. */
