...
import { requireFounderUser } from "@/lib/require-active-user";
...
export default async function WorkspacePage() {
  const session = await requireFounderUser();
    ...
    <div className="min-h-screen bg-stone-100 text-stone-950">
      ...
      <main className="mx-auto max-w-4xl px-6 py-16">
        ...
        {!isWorkspaceActive ? (
          <div className="mt-6 rounded-2xl border border-amber-300 bg-amber-50 p-4">
            <p className="text-sm text-amber-900">
              This workspace is {workspace.status}. Venture changes are
              currently disabled.
            </p>
          </div>
        ) : null}
        ...
        <p className="mt-6 text-lg leading-8 text-stone-700">
          A Venture is an idea you&apos;re currently validating. Create one to
          get started, and switch between them any time.
        </p>
